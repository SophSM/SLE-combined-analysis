...
h <- Heatmap(mat, name = "Z-score", col = color_fun, cluster_rows = F, cluster_columns = F, show_row_names = F, show_column_names = F, 
             ...
             row_split = regulon_split, right_annotation =  c(lfc_anno, term_anno))
...
```{r}
library(tidyverse)
# top two regulons up and down and targets
regulon_df <- unique(filtered_DEregulon_targets %>% select(regulon, log2FC, direction))
regulonUP <- regulon_df %>% filter(direction == "Upregulated") %>%
  arrange(desc(log2FC)) %>%  head(3)
regulonUP_df <- filtered_DEregulon_targets %>% filter(regulon %in% regulonUP$regulon ) %>%  arrange(desc(log2FC))


regulonDown <- regulon_df %>% filter(direction == "Downregulated") %>%
  arrange(log2FC)%>%  head(3)

regulonDown_df <- filtered_DEregulon_targets %>% filter(regulon %in% regulonDown$regulon ) %>%  arrange(desc(log2FC))

smallRegulon <- rbind(regulonUP_df, regulonDown_df )

smallRegulon <- smallRegulon %>% left_join(parentTerms %>% select(regulon, Small), by = "regulon") 

dim(smallRegulon)
small_counts <- zscore[smallRegulon$geneName, , drop = FALSE]
...
col_lfc <- colorRamp2(c(min(smallRegulon$log2FC), 0, max(smallRegulon$log2FC)), c("blue", "white", "red"))

lfc_anno = rowAnnotation("log2FC" = smallRegulon$log2FC, col = list("log2FC" = col_lfc))
unique(smallRegulon$Small)
term_col <- c("Cellular transport" = "#d6b3f2", "Response to stimulus" = "#f0b800", 
  "Immune cell activation" = "#0f5203","Immune system process" = "#ffad7a")

term_anno <- rowAnnotation("Regulon associated process" = smallRegulon$Small,
                           col = list("Regulon associated process" = term_col), 
                           show_annotation_name = F)

smallRegulon$regulon <- factor(smallRegulon$regulon, levels = unique(smallRegulon$regulon))

regulon_split = data.frame("Regulon" = smallRegulon$regulon)
regulon_split$Regulon <- factor(regulon_split$Regulon, levels = unique(smallRegulon$regulon))

sample_split = data.frame("Sample" = ordered_samples$DISEASE)
color_fun <- colorRamp2(c(min(mat2),-2, 0, 2, max(mat2)), c('#086ec2',"lightblue1", 'white', 'red','darkred'))

h <- Heatmap(mat2, name = "Z-score", col = color_fun, cluster_rows = T, cluster_columns = F, show_row_names = F, show_column_names = F, 
             top_annotation = ha, column_split = sample_split, 
             row_split = regulon_split, right_annotation =  c(lfc_anno,term_anno), row_title_rot = 0, show_row_dend = F)

png(filename = glue::glue("{results_dir}/heatmapSmall_targetsRegulons.png"), height = 20, width = 20, units = "cm", res = 300)
